# 부산 소규모 초등학교 통합 교육여건 시뮬레이터

소규모 공립초등학교 A를 다른 초등학교 B와 통합한다고 가정할 때, **학교 안 교육자원**과 **학교 밖 교육접근성**이 어떻게 변하는지 보여주는 Streamlit POC입니다. 같은 A의 후보 안에서 현재 입력자료와 시뮬레이션 규칙상 최선의 수용학교와 대안 순위를 추천합니다.

## 현재 분석 범위

- 모집단: 2025-10-01 부산 공립 초등학교 운영 본교 296개교
- 소규모 분석대상: 부산교육청 2026년 학교운영비 학생수 기준 적용 92개교
- GIS 사용 가능: 전체 294개교, 소규모 90개교
- 후보 규칙: A 학교점에서 직선거리 3km 이하인 다른 초등학교 B
- 후보쌍: 1,481건, 후보가 있는 A 83개교, 없는 A 7개교
- 교육자원 가정: A 학생 전원이 이동하고 일반학급은 2025 부산 학생배치지표 25명으로 학년별 재편성, 교실·교지는 B의 현재 규모 유지. 교원은 B 현재 현원·A+B 현재 현원 합·학급 수 회귀 참고값을 분리 표시
- 접근성 가정: A 통학구역 안에 재학생 수만큼 균일 무작위 가상점을 만들고 반복하여 A/B 학교점까지 직선거리 비교
- 추천순위: 필요 일반학급이 B 일반교실 수를 넘지 않는 후보를 우선하고, 28명 과밀 참고선 확인 후 교육자원 4개·접근성 2개의 최약 지표 백분위와 파레토 여부로 최선안과 대안을 정렬

## 대시보드 사용 흐름

1. 92개 소규모학교 중 통합으로 폐교한다고 가정할 학교 A를 고릅니다.
2. 지도에서 A 반경 3km 안의 수용학교 후보를 확인합니다. 소규모 후보와 일반 후보는 색으로 구분됩니다.
3. 추천 순위에서 최선안과 대안 두 곳을 먼저 확인합니다.
4. 추천 카드, 지도 점 또는 수용학교 B 목록에서 상세 분석할 학교를 선택합니다.
5. 지도는 A와 B에 초점을 맞추고, 화면 아래에서 교육자원과 교육접근성의 Before/After 변화를 보여줍니다. 일반학급은 학년별 일반학생 합계를 25명으로 나눠 올림하고, 수용학교 현재 학급·두 학교 현재 학급 합·재편성 후 필요 학급을 함께 보여줍니다.
6. 교육자원 스타차트에서 선택 시나리오를 같은 A학교의 후보군과 전체 1,481개 후보 시나리오에 각각 비교합니다.
7. 기본 화면은 핵심 결과만 표시하며, `상세 분석 전체 펼치기`를 켜면 후보 수 요약과 교육자원·접근성의 모든 세부 분석을 표시합니다.

후보가 없는 학교와 2025년 학교 마스터에 있으나 GIS 조인이 되지 않은 학교도 A 목록에 남겨 두며, 해당 사유를 화면에서 별도로 안내합니다.

스타차트의 네 축은 학급·교원·교실 부담과 학생 1인당 교지면적의 **지표별 유리한 방향 백분위**입니다. 축을 합산한 종합점수는 만들지 않으며, 학생 수는 교육자원 여유와 방향이 다른 규모 지표이므로 스타차트에서 제외하고 원수치 표에만 표시합니다.

추천순위도 임의 가중합을 사용하지 않습니다. 필요 일반학급이 수용학교 일반교실 수 이하인 후보를 우선하고 28명 과밀 참고선을 함께 확인한 뒤, 일반학급당 학생 수·교원 1인당 학생 수·학생/교실·학생 1인당 교지면적·평균 추가 접근거리·최대 추가 접근거리의 유리한 방향 백분위를 계산합니다. 가장 낮은 백분위부터 차례로 비교해 약점이 가장 작은 후보를 최선안으로 추천하며 파레토 비지배 여부를 함께 표시합니다.

이 순위는 한 지표의 큰 장점이 다른 지표의 큰 손실을 덮지 못하게 만든 **현재 입력자료와 가정 안에서의 최선안**입니다. 회귀 교원 참고값은 공식 배치값이 아니므로 순위에 사용하지 않으며, 통학버스·교원 재배치·지역사회 의견은 후속 정책검토에서 반영해야 합니다.

## 실행

Ubuntu/WSL에서 저장소 루트 기준으로 실행합니다.

```bash
uv sync
uv run python -m scripts.build_assets
uv run pytest
uv run python -m scripts.validate_pipeline --all-scenarios
uv run streamlit run app.py
```

이미 `data/processed/`가 포함된 배포본은 원자료 없이 마지막 세 명령만 실행할 수 있습니다.

## Streamlit Community Cloud

저장소 루트의 `app.py`를 엔트리포인트로 배포합니다. 시크릿·환경변수는 필요 없습니다. Advanced 설정에서 Python 버전은 **3.12**로 고릅니다. 의존성은 `uv.lock`만 사용하므로 `requirements.txt`는 두지 않습니다.

교원 회귀모델의 적용가치 검증 노트북과 결과표는 다음 명령으로 다시 생성합니다.

```bash
uv run python scripts/build_teacher_model_notebook.py
uv run python scripts/build_teacher_model_report.py
```

분석 결론은 회귀 예측을 통합 후 정원으로 사용하지 않고, 규칙 기반 교원 산정의 참고 범위와 이상치 점검에만 사용하는 것입니다.

## 프로젝트 구조

```text
app.py                         Streamlit POC
src/preprocessing.py           EDA 모집단·학교 마스터 생성
src/gis_preprocessing.py       학교 위치·공식 통학구역 조인
src/candidate_generator.py     3km 후보쌍 생성
src/class_formation.py         25명 기준 학년별 일반학급 재편성
src/resource_simulator.py      재편성 학급+고정자원 Before/After 계산
src/resource_benchmark.py      후보군·전체 시나리오 상대 백분위 계산
src/scenario_ranking.py        후보별 최선안·대안 추천순위 계산
src/teacher_model.py           학급 수 기반 교원 관측패턴 참고모형
src/accessibility_simulator.py 통학구역 균일 가상점 반복·직선거리 변화
src/scenario_engine.py         후보 검증과 두 분석의 결합
scripts/build_assets.py        원자료 → 배포 데이터
scripts/validate_pipeline.py   실제 대표 시나리오 회귀검증
scripts/build_teacher_model_notebook.py 교원 회귀 적용가치 검증 노트북 생성·실행
scripts/build_teacher_model_report.py   교원 회귀 기술보고서 원본 생성
notebooks/teacher_regression_value.ipynb 반복 교차검증 분석 노트북
reports/teacher_model/         모델 성능표·예측·기술보고서
tests/                         단위·데이터 계약 테스트
data/raw/                      원자료, Git 제외
data/processed/                Streamlit 배포 데이터, Git 포함
```

## 데이터 계보

| 자료 | 기준일 | 사용 내용 |
|---|---:|---|
| 데이터① 학과·학년·반별 학생수 | 2025-04-01 | 일반/특수 분리 학년별 학생·학급 구조와 일반학급 재편성 |
| 데이터② 학교별 학생·학급·교원 | 2025-10-01 | 모집단, 소규모 기준, 인적 교육자원 |
| 데이터③ 학교별 면적 | 2025-04-01 | 교실 수, 교지면적 |
| 초등학교 통학구역 SHP·표준데이터 | 2026-03-20 | A의 기존 통학구역 |
| 전국 초중등학교 위치표준데이터 | 2026-03-20 | 학교 위치점 |

4월과 10월 값의 차이를 증감 원인으로 해석하지 않습니다. GIS 자료의 운영상태로 2025년 모집단을 다시 필터링하지 않고, 학교명+행정구로 조인 가능한 학교만 공간분석에 사용합니다.

## 배포 데이터

원자료는 크고 앱 실행에 필요하지 않아 `data/raw/`에서만 관리합니다. `python -m scripts.build_assets`가 생성한 `data/processed/`는 Git에 포함하며 Streamlit은 이 가공본만 읽습니다. 파일별 해시와 행수는 `data/processed/dataset_manifest.json`에 기록됩니다.

## 해석상 주의

- 학교 간 거리와 접근거리는 현재 직선거리이며 도로망·경사·통학수단을 반영하지 않습니다.
- 통학구역 안에 균일하게 생성한 가상점이므로 실제 학생 거주분포를 뜻하지 않습니다.
- 통합 이후 교원 재배치, 학급 증설, 시설 확충, 통학버스 제공은 반영하지 않습니다.
- 일반학급은 학년별 `올림((A 일반학생+B 일반학생)/25)`으로 계산합니다. 특수학급은 별도 편성 규칙이 필요해 현재 규모만 분리 표시합니다.
- 25명은 학급을 재편성하는 학생배치지표이고, 28명은 화면의 과밀 참고선일 뿐 재편성 분모나 종합판정 점수로 사용하지 않습니다.
- 계산된 필요 일반학급이 수용학교 일반교실 수를 넘으면 시설 검토 경고를 표시하지만, 자동으로 교실을 늘리지는 않습니다.
- 추천 순위는 같은 A의 3km 후보 안에서만 유효하며 교육자원·접근성 정량지표를 사용한 현재 조건의 최선안입니다.
- 실제 정책 결정에는 학생·학부모·지역주민 의견과 지역사회 여건 검토가 필요합니다.
